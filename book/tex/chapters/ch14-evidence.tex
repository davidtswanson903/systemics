% tex/chapters/ch14-evidence.tex
...
\begin{enumerate}[leftmargin=*, itemsep=0.2em]
  ...
  \item an entailment-intent relation $\Vdash\ \subseteq\ R\times \mathsf{Claim}$ (Definition~\ref{def:receipt-entails}),
  \item a verifier predicate $\Verify:R\times \mathsf{Claim}\to 2$ (Definition~\ref{def:verifier}),
  \item canonicalization data for receipts (a function $\canon:R\to R$ and any required invariants),
  \item the binding-field projection $\bind\circ\canon$ and an explicit statement of binding consistency requirements (Axiom~\ref{ax:E1-verify-bind}),
  \item a claim compatibility predicate $\Compat_\phi$ (Definition~\ref{def:claim-compat}) and a claim constructor for sequential composition ($\diamond$) (Definition~\ref{def:claim-compose}),
  \item a receipt merge operator $\MergeR$ aligned with process composition (Chapter~4) and a statement of merge-closed verification (Definition~\ref{def:merge-closed-verify}),
  \item (optional but recommended) the parallel analogs: parallel claim constructor, compatibility predicate, and merge-closed verification statement for $\parc$.
\end{enumerate}
